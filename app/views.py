...
def sanitize_headers(headers_dict):
    """Usuń znaki spoza latin-1 z nagłówków"""
    clean = {}
    for k, v in headers_dict.items():
        try:
            ...
            clean[k] = v.encode('ascii', 'ignore').decode('ascii')
    return clean
...
def extract_data():
    ...
    response = requests.get(url, headers=safe_headers)
    ...
    recommendation_distr = opinions.recommendation.value_counts(dropna=False).reindex(
        ["Nie polecam", "Polecam", None], fill_value=0
    )
...
def product(product_id: int):
    ...
    return render_template(
        "product.html",
        opinions=opinions.to_html(classes="table table-hover table-bordered table-striped", index=False)
    )
